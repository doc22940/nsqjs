...
var ConnectionConfig, ConnectionState, Debug, EventEmitter, FrameBuffer, Message, NSQDConnection, NodeState, WriterConnectionState, WriterNSQDConnection, fs, net, os, tls, version, wire, zlib, _, _ref,
...
NSQDConnection = (function(_super) {
  ...
  NSQDConnection.prototype.startTLS = function(callback) {
    var event, options, tlsConn, _i, _len, _ref,
      _this = this;
    _ref = ['data', 'error', 'close'];
...
ConnectionState = (function(_super) {
  ...
  ConnectionState.prototype.states = {
    ...
    IDENTIFY_COMPRESSION_CHECK: {
      Enter: function() {
        var deflate, snappy, _ref;
        _ref = this.identifyResponse, deflate = _ref.deflate, snappy = _ref.snappy;
        if (deflate) {
    ...
    ERROR: {
      Enter: function(err) {
        ...
        errorCode = (_ref = err.split(/\s+/)) != null ? _ref[1] : void 0;
    ...
    CLOSED: {
      Enter: function() {
        var cb, err, _i, _len, _ref;
        if (!this.conn) {
          return;
        }
        err = new Error('nsqd connection closed');
        _ref = this.conn.messageCallbacks;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          cb = _ref[_i];
          if (typeof cb === "function") {
            cb(err);
